...
from shared.format_utils import format_comma
from shared.base_estate_widget import BaseEstateWidget


class CommercialEstateWidget(BaseEstateWidget):
    """Commercial property (상가) management widget."""
    ...
    category_config = {
        "상가": {
            "G2": ["준공연도", "건물규모", "용도지역", "대지면적(㎡)", "건축면적(㎡)",
                    "연면적(㎡)", "주구조", "주용도", "거래유형", "거주형태"],
    ...
    def _update_detail_visibility(self, type_name):
        self.detail_group.setVisible(type_name == "상가")

    def _connect_totals_triggers(self):
        if "거래가(만원)" in self.all_fields["G3"]:
            self.all_fields["G3"]["거래가(만원)"].textChanged.connect(self.calculate_detail_totals)
    ...
    def calculate_detail_totals(self):
        ...
        if not price_field:
            price_field = self.all_fields.get("G3", {}).get("거래가(만원)")
