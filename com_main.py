...
class EstateMaster_V13_6(QWidget):
    def __init__(self):
        ...
        self.category_config = {
            "상가": {
                "G2": ["준공연도","건물규모", "용도지역", "대지면적(㎡)", "건축면적(㎡)", "연면적(㎡)", "주구조", "주용도", "거래유형", "거주형태"],
                "G3": ["거래가(만원)", "자기자본", "대출금", "대출이자율(%)", "수익율(%)"],
    ...
    def initUI(self):
        ...
        self.commercial_detail_group = self.create_commercial_detail_ui()
        prop_layout.addWidget(self.commercial_detail_group)
...
궁금하신 사항은 언제든 문의주세요.\n{Config.SNS_FOOTER}"""
    ...
    def on_type_changed(self):
        ...
        # 상가일 경우 상세 테이블 노출
        self.commercial_detail_group.setVisible(type_name == "상가")
        ...
        # 실시간 포맷팅 연결 (콤마 및 하이픈)
        if "거래가(만원)" in self.all_fields["G3"]:
            self.all_fields["G3"]["거래가(만원)"].textChanged.connect(lambda _: self.format_comma("G3", "거래가(만원)"))
            self.all_fields["G3"]["거래가(만원)"].textChanged.connect(self.calculate_commercial_totals)
