...
class EstateMaster_V13_6(QWidget):
    ...
    def initUI(self):
        ...
        self.house_detail_group = self.create_house_detail_ui()
        prop_layout.addWidget(self.house_detail_group)
...
궁금하신 사항은 언제든 문의주세요.\n{Config.SNS_FOOTER}"""
    ...
    def load_data(self):
        ...
        try:
            ...
            with pd.ExcelFile(self.db_path, engine='openpyxl') as reader:
                ...
                # 2. 주택 상세 테이블 조회
                house_detail_sheet = "주택상세"
                if house_detail_sheet in reader.sheet_names:
                    df_detail = pd.read_excel(reader, sheet_name=house_detail_sheet)
                    rows = df_detail[df_detail['매물ID'] == m_id]
                    self.table_house.setRowCount(0)
                    headers = ["층", "호실", "용도", "면적", "전세보증금", "월세보증금", "월세", "옵션", "현재상태", "임차만기일"]
                    for _, d_row in rows.iterrows():
                        r_idx = self.table_house.rowCount()
                        self.table_house.insertRow(r_idx)
                        for c_idx, h in enumerate(headers):
                            val = str(d_row.get(h, ""))
                            self.table_house.setItem(r_idx, c_idx, QTableWidgetItem(val if val != "nan" else ""))
    ...
    def on_type_changed(self):
        ...
        self.house_detail_group.setVisible(type_name == "주택")
        ...
        g4_labels = ["소유자", "성별", "소유자연락처", "통신사", "연락처구분", "연락처", "비번(현관/집)", "조정가", "중개사메모1", "중개사메모2"]
        ...
        if "거래가" in self.all_fields["G3"]:
            ...
            self.all_fields["G3"]["거래가"].textChanged.connect(self.calculate_house_totals)
        ...
        if target_table:
            ...
            for r in range(target_table.rowCount()):
                ...
                for c in range(target_table.columnCount()):
                    item = target_table.item(r, c); val = item.text() if item else ""
